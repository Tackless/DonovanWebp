...
@section('contenido')
    @if ($post->categoria === 'Front')
        <h1 class="p-6 text-4xl md:text-7xl text-white font-bold bg-pink-500 mb-6 shadow-lg">@yield('titulo')</h1>
    @elseif ($post->categoria === 'Back')
        <h1 class="p-6 text-4xl md:text-7xl text-white font-bold bg-blue-600 shadow-lg">@yield('titulo')</h1>
    @else
        <h1 class="p-6 text-4xl md:text-7xl text-black font-bold bg-yellow-300 mb-6 shadow-lg">@yield('titulo')</h1>
    @endif

    @if ($post->categoria === 'Front' || $post->categoria === 'Js' )
    ...
    <div class="p-5 flex flex-col xl:flex-row justify-center gap-5">
        @if ($post->categoria === 'Front' || $post->categoria === 'Js' )
            <div class=" shadow bg-white rounded p-5 mb-5 xl:w-4/6">
                <p class="text-2xl">
                    {{ $post->descripcion }}
                </p>
            </div>
        @endif
        <div class="xl:flex flex-col justify-center xl:w-2/6">  
            @if ($post->link)
                <a target="_blank" href="{{ $post->link }}" class="">
                    <div class=" shadow-xl bg-green-400 hover:bg-green-500 duration-500 p-5 mb-5 col-start-2 row-start-3 rounded-full flex justify-center items-center">
                        ...
                        <p class="text-black ml-4">Link de la página</p>
                    </div>
                </a>
            @endif
            ...
            @auth
                <form action="{{ route('posts.destroy', ['post' => $post]) }}" method="POST">
                    ...
                    <input type="submit" value="Eliminar Post" class=" shadow-xl bg-red-600 hover:bg-red-800 text-white font-bold duration-500 p-5 mb-5 col-start-2 row-start-3 rounded-full flex justify-center items-center cursor-pointer">
                </form>
            @endauth
        </div>
    </div>
